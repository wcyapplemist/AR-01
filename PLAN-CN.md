...
| **P3** | QR Code 扫描模块 | 摄像头 + jsQR 解码 + 内容验证，演示页 `/qr` | 2–3 天 | **已完成** |
...
### Phase 3：QR Code 扫描模块 — 已完成

> 摄像头 + jsQR 解码 + 内容验证。独立演示页 `/qr`。

- [x] P3.1：定义 QR 类型和接口（`QRResult`、`QRContent`、`ScanOptions`）
- [x] P3.2：实现 `CameraService`（后置摄像头访问、流管理、平台差异处理）
- [x] P3.3：实现 `QRDecoder` 服务（jsQR 集成、帧提取、解码管线）
- [x] P3.4：实现 `QRContentValidator` 服务（内容格式验证、校验验证）
- [x] P3.5：实现 `useQRScanner` React hook（启停扫描、结果流）
- [x] P3.6：实现 `QRScannerOverlay` 组件（取景器 UI、扫描区域高亮、结果显示）
- [x] P3.7：构建 `/qr` 演示页面（摄像头预览 + 扫描叠加层）
...
| **M4：QR 扫描可用** | P3 | `/qr` 页面可扫描并解析 QR Code，带叠加层 | **已完成** |
